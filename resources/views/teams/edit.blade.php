<x-app-layout>
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Modifier l'équipe</h1>
        ...
        <form method="POST" action="{{ route('teams.update', $team) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" maxlength="255"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 @error('description') border-red-500 @enderror">{{ old('description', $team->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
