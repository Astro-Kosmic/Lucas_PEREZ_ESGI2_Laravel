<x-app-layout>
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Modifier {{ $pokemon->name }}</h1>

    <div class="bg-white rounded-xl shadow p-8 max-w-2xl">
        <form method="POST" action="{{ route('admin.pokemons.update', $pokemon) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $pokemon->name) }}" maxlength="100" required
            ...
            <div>
                ...
                <textarea name="description" rows="3" required
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 @error('description') border-red-500 @enderror">{{ old('description', $pokemon->description) }}</textarea>
            ...
            <div class="grid grid-cols-3 gap-4">
                @foreach(['hp' => 'HP', 'attack' => 'Attaque', 'defense' => 'Défense', 'special_attack' => 'Sp. Atk', 'special_defense' => 'Sp. Déf', 'speed' => 'Vitesse'] as $field => $label)
                    <div>
                        ...
                        <input type="number" name="{{ $field }}" value="{{ old($field, $pokemon->$field) }}" min="1" max="255" required
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="height" value="{{ old('height', $pokemon->height) }}" step="0.01" min="0.1" required
                ...
                <div>
                    ...
                    <input type="number" name="weight" value="{{ old('weight', $pokemon->weight) }}" step="0.01" min="0.1" required
                ...
                <div>
                    ...
                    <input type="number" name="generation" value="{{ old('generation', $pokemon->generation) }}" min="1" max="9" required
            ...
            <div>
                ...
                <input type="url" name="sprite_url" value="{{ old('sprite_url', $pokemon->sprite_url) }}" required
                ...
                <select name="types[]" multiple size="6"
                    ...
                    @foreach($types as $type)
                        <option value="{{ $type->id }}"
                                @selected(in_array($type->id, old('types', $selectedTypeIds)))>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </select>
